...
cartdata = ['<table><tr><th>Product Name</th><th>Quantity</th><th>Price</th><th>Total</th><th><button onclick="delElemenrAll()">✘</button></th></tr>'];
  ...
  for (i=0; i < inames.length; i++){
    ...
    cartdata += '<tr>' + 
                       '<td>' + inames[i] + '</td>' +
                        '<td>' + iqtyp[i] + '</td>' + 
                        '<td>' + iprice[i] + '</td>' +
                        '<td>' + iqtyp[i] * iprice[i] + '</td>' +
                         '<td>' + 
                             '<button onclick="delElement(' + i + ')">✘2</button>'+
                        '</td>' + 
                '</tr>';
  }
